...
function print() {
  ...
  var fruits = ['banana', 'apple', 'orange', 'guava']; // Map

  var fruitsFormatted = fruits.map(function (fruit) {
    return {
      name: fruit
    };
  }); // Filter

  var filtered = fruits.filter(function (fruit, index) {
    return index > 1;
  });

  var copied = _toConsumableArray(filtered);

  copied.push('pineapple');
  var copied1 = filtered.concat([]);
